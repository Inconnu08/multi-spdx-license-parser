from spdx_license_matcher.build_licenses import get_license_text
from spdx_license_matcher.normalize import normalize
from spdx_license_matcher.sorensen_dice import get_dice_coefficient, diffRatio
...
def get_close_matches(inputText, licenseData, threshold=0.9):
    ...
    print('get close matches', matches)
    return matches
...
def get_matching_string(matches, inputText):
    ...
    if not matches:
        ...
        # print('elif all(score == 1.0 for score in list(matches.values())):')
        ...
        for licenseID in matches:
            ...
            print('ID', licenseID)
            listedLicense = get_license_text(licenseID)
            # print('get_matching_string', licenseID)
            if not licenseID == 'BSD-4-Clause-Shortened' and licenseID == 'curl':
                isTextStandard = checkTextStandardLicense(listedLicense, inputText)
                if not isTextStandard:
                    # print('istextstd bool', isTextStandard)
